    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col relative overflow-x-hidden selection:bg-blue-600 selection:text-white transition-all duration-300">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10 flex-1 mb-16">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* Quick ERP Portal Login Card (Highly visible on-page sign-in form) */}
          <div className="bg-gradient-to-br from-slate-900/95 via-slate-900/80 to-slate-950 border border-blue-500/15 rounded-3xl p-6 space-y-5 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 inset-x-0 h-[3px] bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-600" />
            
            <div className="space-y-1">
              <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-1.5 leading-none">
                <Lock size={11} className="text-blue-500 stroke-[2.5]" /> Secure Member Access
              </span>
              <h4 className="text-sm font-black text-white uppercase tracking-wider">
                ERP Portal Authorization
              </h4>
            </div>

            {errorVisible && (
              <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-[10px] text-center font-bold animate-pulse">
                {errorVisible}
              </div>
            )}
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="inline-username" className={cn("text-slate-350 text-[9px] font-black uppercase tracking-wider flex justify-between items-center", formErrors.username && "text-red-400")}>
                  <span>Username / Mail ID</span>
                  {formErrors.username && <span className="lowercase font-extrabold text-[8px]">{formErrors.username}</span>}
                </Label>
                <Input 
                  id="inline-username" 
                  type="text" 
                  ...
                  className="bg-slate-950 border-slate-850 text-white placeholder:text-slate-750 h-9.5 text-xs rounded-xl focus:border-blue-500 focus:ring-blue-500/10 font-medium"
                />
              </div>

              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <Label htmlFor="inline-password" className={cn("text-slate-350 text-[9px] font-black uppercase tracking-wider flex items-center gap-1", formErrors.password && "text-red-400")}>
                    <span>Password Prefix</span>
                    {formErrors.password && <span className="lowercase font-extrabold text-[8px]">- {formErrors.password}</span>}
                  </Label>
                  <button 
                    type="button" 
                    onClick={() => { setShowForgot(true); setIsLoginModalOpen(true); }}
                    className="text-[9px] text-blue-400 hover:text-blue-300 font-extrabold uppercase tracking-wider cursor-pointer"
                  >
                    Forgot?
                  </button>
                </div>
                <Input 
                  ...
                  className="bg-slate-950 border-slate-850 text-white placeholder:text-slate-750 h-9.5 text-xs rounded-xl focus:border-blue-500 focus:ring-blue-500/10 font-semibold"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label className="text-slate-500 text-[8px] font-black uppercase tracking-wider">Secured Node</Label>
                  <div className="h-8.5 flex items-center px-2.5 rounded-lg bg-slate-950 border border-slate-900 text-slate-400 text-[9px] font-bold uppercase tracking-widest overflow-hidden truncate">
                    {currentSchoolObj ? getSafeNameVal(currentSchoolObj).replace("SCANiD ", "") : "PORTAL"}
                  </div>
                </div>

                <div className="space-y-1">
                  <Label className="text-slate-400 text-[8px] font-black uppercase tracking-wider flex items-center gap-1">
                    <Calendar size={8} className="text-blue-400" /> Session
                  </Label>
                  <Select value={selectedYear} onValueChange={(v) => { setSelectedYear(v); setFormErrors(prev => ({...prev, year: ""})); }}>
                    <SelectTrigger className="bg-slate-950 border-slate-850 text-white h-8.5 text-[10px] rounded-lg">
              ...
              <Button 
                type="submit" 
                disabled={loading}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white h-10.5 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-500/10 mt-3 rounded-xl transition-all duration-300 hover:-translate-y-0.5 active:scale-95 disabled:opacity-50"
              >
                {loading ? "Authorizing..." : "Decrypt ERP Portal"}
              </Button>
            </form>
          </div>
