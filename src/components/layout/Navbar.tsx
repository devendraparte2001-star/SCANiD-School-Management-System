...
export default function Navbar({ user, onLogout, onUserUpdate, toggleSidebar }: NavbarProps) {
  ...
  return (
      ...
      <Select value={user.schoolId?.toString() || ""} onValueChange={handleSchoolChange}>
        <SelectTrigger className="w-[180px] h-9 bg-slate-50 border-slate-200 text-xs font-bold rounded-lg focus:ring-2 focus:ring-blue-500/10">
          <div className="flex items-center gap-2 truncate">
            ...
            <SelectValue placeholder={t("Select School Branch")}>
              {user.schoolId ? (user.schoolId === "all" ? t("Global Admin View") : t(schools.find(s => s.id.toString() === user.schoolId.toString())?.name || "", schools.find(s => s.id.toString() === user.schoolId.toString())?.name)) : undefined}
            </SelectValue>
          </div>
        </SelectTrigger>
        <SelectContent className="rounded-xl border-slate-100 shadow-xl">
          <SelectItem value="" className="text-xs italic text-slate-400">{t("Select School Branch")}</SelectItem>
                  <SelectItem value="all" className="text-xs font-black text-blue-600">{t("Global Admin View")}</SelectItem>
                  {Array.isArray(schools) && schools.map(s => (
                    <SelectItem key={s.id || Math.random()} value={s.id ? s.id.toString() : ""} className="text-xs font-medium">{t(s.name, s.name)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="bg-slate-50 px-3 py-1.5 rounded-lg hidden sm:flex items-center gap-2 border border-slate-100">
              ...
              <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider truncate max-w-[140px]">
                {t(user.schoolName || "Institutional Access", user.schoolName || "Institutional Access")}
              </span>
            </div>
          )}
        ...
        <SelectTrigger className="w-[140px] h-9 bg-blue-50 border-blue-100 text-xs font-black text-blue-700 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <SelectValue placeholder={t("Select Academic Year")}>
              {user.academicYearId ? t(academicYears.find(y => y.id.toString() === user.academicYearId.toString())?.name || "", academicYears.find(y => y.id.toString() === user.academicYearId.toString())?.name) : undefined}
            </SelectValue>
          </div>
        </SelectTrigger>
        <SelectContent className="rounded-xl border-slate-100 shadow-xl">
          <SelectItem value="" className="text-xs italic text-slate-400">{t("Select Academic Year")}</SelectItem>
                {Array.isArray(academicYears) && academicYears
                  ...
                  .map(y => (
                    <SelectItem key={y.id || Math.random()} value={y.id ? y.id.toString() : ""} className="text-xs font-bold">
                      {t(y.name, y.name)} {(y.isCurrent || y.IsCurrent) ? "★" : ""}
                    </SelectItem>
                  ))
                }
              </SelectContent>
    ...
    <div className="flex items-center gap-4">
        ...
        <DropdownMenu>
          <SimpleTooltip content={t("Notifications")} side="bottom" nativeButton={true}>
            <DropdownMenuTrigger asChild nativeButton={true}>
              <Button 
                variant="ghost" 
                size="icon" 
          ...
          <DropdownMenuContent align="end" className="w-80 p-0 overflow-hidden bg-white">
            <div className="flex items-center justify-between p-4 bg-slate-50/50 border-b border-slate-100">
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-medium">{t("You have")} {unreadCount} {t("unread alerts")}</p>
              </div>
              {unreadCount > 0 && (
                <button 
            ...
            <div className="max-h-[350px] overflow-y-auto">
              {Array.isArray(notifications) && notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-xs font-medium text-slate-500">{t("All caught up!")}</p>
                  <p className="text-[10px] text-slate-400 mt-1">{t("No new notifications at the moment.")}</p>
                </div>
              ) : (
        ...
        <DropdownMenu>
            <SimpleTooltip content={t("User Menu")} side="bottom" nativeButton={false}>
            <DropdownMenuTrigger
              nativeButton={false}
              render={
                <div className={cn("flex items-center gap-3 px-3 py-2 rounded-md hover:bg-slate-50 transition-colors text-slate-600 cursor-pointer border-none bg-transparent outline-none")}>
                  <div className="text-right hidden sm:block">
                    <p className="text-sm font-bold text-slate-900 leading-tight">{t(user.name, user.name)}</p>
                  </div>
          ...
          <DropdownMenuContent align="end" className="w-64 p-0 overflow-hidden bg-white">
            <div className="flex items-center gap-3 p-4 bg-slate-50/50 border-b border-slate-100">
              ...
              <div className="flex flex-col space-y-0.5 min-w-0">
                <p className="text-sm font-bold text-slate-900 truncate leading-none">{t(user.name, user.name)}</p>
